...
elseif strcmp(command_str,'print')
    ...
    if freq~=0 %HG EDITED 9/30/19
        fName= strrep(horzcat('Q',animal, '_',num2str(freq),'Hz_',ChinFile,'_',date), '-', '_');
    else
        fName= strrep(horzcat('Q',animal, '_Click_',ChinFile,'_',date), '-', '_');
    end
    ...
    uiwait(msgbox(sprintf('File Saved: %s',fName)))
